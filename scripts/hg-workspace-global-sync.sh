...
Options:
  ...
  --check                     Exit non-zero if managed global home-overlay
                              state is stale
...
global_claude_skill_targets_managed() {
  [[ -d "$CLAUDE_SKILLS_DIR" ]] || return 1

  local dir
  while IFS= read -r dir; do
    skill_dir_is_replaceable "$dir" && return 0
  done < <(find "$CLAUDE_SKILLS_DIR" -mindepth 1 -maxdepth 1 -type d -print 2>/dev/null | sort)

  return 1
}

claude_mcp_overlay_is_managed() {
  [[ -f "$CLAUDE_JSON_PATH" ]] || return 1
  jq -e --arg project "$CLAUDE_PROJECT_KEY" '
    (((.projects // {})[$project].mcpServers // {}) | keys | map(startswith("studio_")) | any)
  ' "$CLAUDE_JSON_PATH" >/dev/null 2>&1
}

codex_mcp_overlay_is_managed() {
  [[ -f "$CODEX_CONFIG_PATH" ]] || return 1
  grep -Fqx "$START_MARKER" "$CODEX_CONFIG_PATH" || grep -Fqx "$LEGACY_START_MARKER" "$CODEX_CONFIG_PATH"
}

gemini_mcp_overlay_is_managed() {
  [[ -f "$GEMINI_SETTINGS_PATH" ]] || return 1
  jq -e '
    ((.mcpServers // {}) | keys | map(startswith("studio-")) | any)
  ' "$GEMINI_SETTINGS_PATH" >/dev/null 2>&1
}

gemini_home_doc_is_managed() {
  [[ -f "$GEMINI_HOME_DOC_PATH" ]] || return 1
  grep -Fqx "$GEMINI_START_MARKER" "$GEMINI_HOME_DOC_PATH"
}

managed_home_overlay_target() {
  local target_kind="$1"
  case "$target_kind" in
    claude_skills)   global_claude_skill_targets_managed ;;
    claude_mcp)      home_overlay_stamp_is_managed "claude_mcp" || claude_mcp_overlay_is_managed ;;
    codex_mcp)       home_overlay_stamp_is_managed "codex_mcp" || codex_mcp_overlay_is_managed ;;
    gemini_mcp)      home_overlay_stamp_is_managed "gemini_mcp" || gemini_mcp_overlay_is_managed ;;
    gemini_home_doc) home_overlay_stamp_is_managed "gemini_home_doc" || gemini_home_doc_is_managed ;;
    always)          return 0 ;;
    *)               return 1 ;;
  esac
}

home_overlay_target_path() {
  local target_kind="$1"
  case "$target_kind" in
    claude_mcp)      printf '%s\n' "$CLAUDE_JSON_PATH" ;;
    codex_mcp)       printf '%s\n' "$CODEX_CONFIG_PATH" ;;
    gemini_mcp)      printf '%s\n' "$GEMINI_SETTINGS_PATH" ;;
    gemini_home_doc) printf '%s\n' "$GEMINI_HOME_DOC_PATH" ;;
    gemini_projects) printf '%s\n' "$GEMINI_PROJECTS_PATH" ;;
    *)               return 1 ;;
  esac
}

home_overlay_stamp_path() {
  local target_kind="$1"
  local target_path
  target_path="$(home_overlay_target_path "$target_kind")" || return 1
  printf '%s/.hg-workspace-global-sync-%s.json\n' "$(dirname "$target_path")" "$target_kind"
}

home_overlay_stamp_is_managed() {
  local target_kind="$1"
  local stamp_path
  stamp_path="$(home_overlay_stamp_path "$target_kind")" || return 1
  [[ -f "$stamp_path" ]]
}

mark_managed_home_overlay() {
  local target_kind="$1"
  local stamp_path target_path
  stamp_path="$(home_overlay_stamp_path "$target_kind")" || return 0
  target_path="$(home_overlay_target_path "$target_kind")" || return 0
...
sync_skill_dir() {
  ...
  if [[ "$MODE" == "check" ]] && ! global_claude_skill_targets_managed; then
    return 0
  fi
...
sync_rendered_file() {
  ...
  if [[ "$MODE" == "check" ]] && ! managed_home_overlay_target "$target_kind"; then
    rm -f "$rendered_path"
    return 0
  fi

  if [[ -f "$current_path" ]] && cmp -s "$current_path" "$rendered_path"; then
    if [[ "$MODE" == "write" && "$target_kind" != "always" ]]; then
      mark_managed_home_overlay "$target_kind"
    fi
    rm -f "$rendered_path"
    return 0
  fi
...
sync_managed_workspace_tools() {
  ...
  sync_rendered_file "$CLAUDE_JSON_PATH" "$claude_output_json" "Synced workspace Claude MCP overlay" "claude_mcp"
  ...
  sync_rendered_file "$GEMINI_SETTINGS_PATH" "$gemini_output_json" "Synced workspace Gemini MCP overlay" "gemini_mcp"
  ...
  sync_rendered_file "$CODEX_CONFIG_PATH" "$codex_output_file" "Synced workspace Codex MCP block" "codex_mcp"
}
...
sync_gemini_home_context() {
  ...
  sync_rendered_file "$GEMINI_HOME_DOC_PATH" "$gemini_output_file" "Synced Gemini home memory" "gemini_home_doc"
