...
strip_mcp_tools() {
  ...
  while IFS= read -r line; do
    ...
    if [[ $in_frontmatter -eq 1 ]]; then
      # Drop disable-model-invocation (Claude Code-specific; other runtimes ignore
      # or may warn on unknown fields)
      if [[ "$line" =~ ^disable-model-invocation: ]]; then
        continue
      fi
...
for source_file in "$CLAUDE_COMMANDS"/*.md; do
  ...
  description="$(extract_description "$source_file" | transform_stdin)"
...
if [[ -d "$CLAUDE_SKILLS" ]]; then
  for skill_dir in "$CLAUDE_SKILLS"/*/; do
    ...
    # Rewrite Claude-only body tokens (slash commands, ScheduleWakeup, CronCreate,
    # $ARGUMENTS) into provider-neutral prose so the Codex/Agents mirrors read
    # cleanly under runtimes that lack those tools.
    staged_transformed="$tmpdir/skill_transformed_${name}.md"
    transform_body "$staged_named" > "$staged_transformed"
    ...
    awk -v marker="<!-- GENERATED BY hg-global-skill-sync.sh FROM ~/.claude/skills/${name}/SKILL.md; DO NOT EDIT -->" '
      ...
      { print }
    ' "$staged_transformed" > "$staged_codex"
