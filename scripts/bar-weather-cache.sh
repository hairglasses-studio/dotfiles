#!/usr/bin/env bash
# bar-weather-cache.sh — Cache writer for the Ironbar weather widget
#
# Fetches weather from wttr.in and writes a single line to /tmp/bar-weather.txt.
# Run by bar-weather.timer (every 30 minutes); Ironbar reads the cache file
# with `cat`, never blocking the GTK main loop on network I/O.
#
# Cache format: plain text label, e.g. " 14° Partly cloudy"
# On failure: leaves any existing cache intact (stale data preferred over blank).

set -euo pipefail

CACHE_FILE="/tmp/bar-weather.txt"
TMPFILE="$(mktemp /tmp/bar-weather.XXXXXX)"
