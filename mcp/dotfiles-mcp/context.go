				...
				return []mcp.ResourceContents{
					mcp.TextResourceContents{
						...
						Text: strings.Join([]string{
							...
							"2. Use `desktop_capabilities`, `desktop_snapshot`, `desktop_list_windows`, `desktop_target_windows`, `desktop_find`, `desktop_find_all`, `desktop_focus_window`, `desktop_focus`, `desktop_read_value`, `desktop_set_text`, `desktop_set_value`, or `desktop_act` first when the target is a labeled UI element or semantic window; use `hypr_list_windows` or `hypr_get_monitors` when the task is scene- or window-oriented.",
