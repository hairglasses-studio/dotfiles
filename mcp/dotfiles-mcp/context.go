				...
				return []mcp.ResourceContents{
					mcp.TextResourceContents{
						...
						Text: strings.Join([]string{
							...
							"2. Use `desktop_capabilities`, `desktop_snapshot`, `desktop_target_windows`, or `desktop_find` first when the target is a labeled UI element; use `hypr_list_windows` or `hypr_get_monitors` when the task is scene- or window-oriented.",
							...
							"4. Use `session_connect` or `session_start` when the task needs an explicit session handle, then prefer `session_screenshot`, `session_launch_app`, or clipboard/session helpers over ad-hoc shell reconstruction.",
...
func (m *dotfilesResourceModule) overviewMarkdown() string {
	...
	return fmt.Sprintf(strings.Join([]string{
		...
		"Highest-value paths:",
		"",
		"- Desktop control: `dotfiles_desktop_status` -> `dotfiles_rice_check` -> `desktop_capabilities` / `desktop_snapshot` / `desktop_target_windows` / `desktop_find` -> `hypr_list_windows` / `hypr_get_monitors` / `hypr_monitor_preset_list` / `hypr_layout_list` -> OCR only if needed -> narrow input, semantic action, or scene restore action.",
