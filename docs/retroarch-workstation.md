...
| `retroarch-playlist-audit` | Walk `~/.config/retroarch/playlists/*.lpl`, flag entries whose `core_path` points at a missing `.so`. `DETECT` and empty `core_path` are treated as intentional ("pick at launch"). Writes JSON to `$XDG_STATE_HOME/retroarch/playlist-audit.json`. Called automatically at the end of `retroarch-complete`. |
