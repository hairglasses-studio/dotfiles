...
- Workspace-owned repo skills staged under `~/.claude/skills/` are source artifacts for Claude and Antigravity routing. `scripts/hg-global-skill-sync.sh` intentionally keeps those repo-owned exports out of `~/.codex/skills/` and `~/.agents/skills/` by default so live portable sessions do not load every repo skill catalog globally.
