...
// Plane — terminal content on a rotating flat plane in 3D space via pinhole camera
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Single plane: z = 0 → t = -fp.z / fv.z
    if (abs(fv.z) < 0.0001) { fragColor = vec4(0.0); return; }
    float t = -cam.focal_point.z / fv.z;

    if (t < 1.0) { fragColor = vec4(0.0); return; }
    ...
    fragColor = texture(iChannel0, uvCoord / res);
}
