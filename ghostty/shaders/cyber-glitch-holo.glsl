...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    float timeSinceCursor = t - iTimeCursorChange;
    float cursorActivity = exp(-timeSinceCursor * 2.0); // decays quickly
    float cursorDist = length(fragCoord - iCurrentCursor.xy);
    ...
    float r = texture(iChannel0, sampleUV + vec2( splitR, 0.0)).r;
    float g = texture(iChannel0, sampleUV).g;
    float b = texture(iChannel0, sampleUV + vec2(-splitB, 0.0)).b;
    ...
    if (corrupted) {
        float noiseVal = _hash(fragCoord * 0.5 + vec2(frameSeed * 100.0, 0.0));
    ...
    // --- Film grain ---
    float grain = _hash(fragCoord + vec2(t * 1000.0, 0.0)) * 0.025;
    color += grain;
