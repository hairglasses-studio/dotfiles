...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Tunable parameters
    const float sc_freq = 0.2;         // Scanline frequency
    const float sc_intensity = 0.6;    // Scanline strength
    const bool grid = false;           // Apply scanlines to X axis too
    const float distortion_offset = 2.0; // Chromatic distortion in pixels
    const int downscale_factor = 2;    // Pixel block size
    const float sph_distance = 500.0;  // Sphere distance for curvature
    const float curvature = 1.5;       // Curvature amount
    const float shadow_cutoff = 1.0;   // Edge shadow start
    const int shadow_intensity = 1;    // Shadow strength (1-5)

    float radius = res.x / curvature;

    // Spherical curvature: project UV through curved surface
    vec2 pixCenter = center;
    vec2 offset = uv - pixCenter;
    vec2 scaled = offset * res; // back to pixel space for curvature math
    ...
    vec2 curvedUv = curved / res + pixCenter;
    ...
    // Downscale: average block of pixels
    vec4 c = vec4(0.0);
    ...
    // Scanlines (using original UV for consistent frequency)
    ...
    // Edge shadow (polynomial darkening)
