// Volumetric clouds — "Protean Clouds" by nimitz (Stanton)
...
vec4 render(vec3 ro, vec3 rd, float time) {
    ...
    for (int i = 0; i < 80; i++) {
        ...
        if (mpv.x > 0.6) {
            ...
            float dif = clamp((den - clamp(map(pos + 0.8).x - 0.3, 0.0, 1.0)) / 0.6, 0.0, 1.0);
            dif += clamp((den - clamp(map(pos + 0.35).x - 0.3, 0.0, 1.0)) / 0.6, 0.0, 1.0) * 0.8;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    bsMo = vec2(sin(iTime * 0.15) * 0.3, sin(iTime * 0.1) * 0.2);

    float time = iTime * 3.0;
    vec3 ro = vec3(0, 0, time);
    ro += vec3(sin(iTime) * 0.5, sin(iTime * 0.6) * 0.5, 0);
    float dspAmp = 0.85;
    ...
    fragColor = vec4(mix(terminal.rgb, col * 0.8, mask), terminal.a);
}
