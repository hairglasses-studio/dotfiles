...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    float sunDist = length((uv - sunCenter) * vec2(1.0, iResolution.x / iResolution.y));
    ...
    if (uv.y < horizonY) {
        ...
        // Scrolling Z lines
        float zLine = smoothstep(lineWidth, 0.0, abs(fract(z * gridDensity - iTime * scrollSpeed) - 0.5) * 2.0);
        // X grid lines
    ...
    float scanline = 1.0 - 0.04 * sin(fragCoord.y * 1.5);
    scene *= scanline;
