...
Camera setup_camera(Camera cam) {
    ...
    // Rotate each basis vector through X, Y, Z
    vec3 tmp;
    // base_x
    tmp = cam.base_x;
    tmp.y = cam.base_x.y*cx - cam.base_x.z*sx; tmp.z = cam.base_x.y*sx + cam.base_x.z*cx; cam.base_x = tmp;
    ...
    // base_y
    tmp = cam.base_y;
    tmp.y = cam.base_y.y*cx - cam.base_y.z*sx; tmp.z = cam.base_y.y*sx + cam.base_y.z*cx; cam.base_y = tmp;
    ...
    // base_z
    tmp = cam.base_z;
    tmp.y = cam.base_z.y*cx - cam.base_z.z*sx; tmp.z = cam.base_z.y*sx + cam.base_z.z*cx; cam.base_z = tmp;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Time: 20-second cycle (picom used ms, we use seconds)
    float time_cyclic = mod(iTime * 0.1, 2.0);

    // Camera preset: rotate around origin
    Camera cam = Camera(
        -wss,
    ...
    // 6 cube faces: +Z, -Z, +X, -X, +Y, -Y at distance wss/2
    float half = wss * 0.5;
    ...
    // Solve ray-plane intersections and sort by distance
    float tVals[6];
    int faceIds[6];
    for (int i = 0; i < 6; i++) {
