...
env_key_instructions = "source \"$HOME/hairglasses-studio/dotfiles/scripts/lib/hg-local-llm.sh\" && hg_local_llm_export_env"
...
if $CHECK; then
  ...
  if $INCLUDE_CODEX_CONFIG && ! cmp -s "${tmpdir}/config.toml" "${REPO_PATH}/.codex/config.toml"; then
    echo "codex-config (drift)" >&2
    exit 1
  fi
  exit 0
fi
...
if $INCLUDE_CODEX_CONFIG && git -C "${REPO_PATH}" ls-files --error-unmatch .codex/config.toml >/dev/null 2>&1 && [[ -f "${REPO_PATH}/.codex/config.toml" ]] && ! cmp -s "${tmpdir}/config.toml" "${REPO_PATH}/.codex/config.toml"; then
  generated_drift=true
fi
...
if $INCLUDE_CODEX_CONFIG && git -C "${REPO_PATH}" ls-files --error-unmatch .codex/config.toml >/dev/null 2>&1 && [[ -n "$(git -C "${REPO_PATH}" status --porcelain -- .codex/config.toml 2>/dev/null)" ]]; then
  tracked_dirty=true
fi
...
if $INCLUDE_CODEX_CONFIG; then
  cp "${tmpdir}/config.toml" "${REPO_PATH}/.codex/config.toml"
fi
EOF
...
@test "hg-provider-settings-sync: sync writes escaped codex local model instructions" {
    write_manifest "demo"
    init_repo "demo"
    write_agents "demo"
    ...
    run grep -F 'env_key_instructions = "source \"$HOME/hairglasses-studio/dotfiles/scripts/lib/hg-local-llm.sh\" && hg_local_llm_export_env"' "${TEST_ROOT}/demo/.codex/config.toml"
    assert_success
}
