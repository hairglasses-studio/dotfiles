...
- [ ] [P3][S] Submit to PulseMCP, Glama, MCP Market directories — standalone mirror/release state repaired on 2026-04-23: `hairglasses-studio/dotfiles-mcp` is public/non-archived, tagged and released at `v1.1.0`, `go install ...@latest` resolves the release, Glama metadata is committed, and projection status is `status=in_sync`. Remaining listing work is external: MCP Market browser review form, optional Glama browser submission or crawler pickup, and PulseMCP after Official MCP Registry publication; the official registry needs a supported package channel such as OCI or MCPB, not just the Go module.
