...
- [x] [P2][S] `tests/repo_smoke.bats` README MCP fleet guard — README now describes the current `dotfiles-mcp`/`mapitall`/`mapping` split and fails if retired standalone `systemd-mcp`, `tmux-mcp`, or `process-mcp` rows are reintroduced as active servers.
- [x] [P2][S] `validate-tool-count-claims.sh` headline coverage — README badge and intro MCP counts now track the live `mcp.json` snapshot instead of stale org-wide `1,400+` wording; the validator scans comma-formatted badge counts and publishing copy.
- [x] [P2][S] Active shader MCP doc guard — `docs/SHADER-PIPELINE.md` and `mcp/dotfiles-mcp/mod_shader.go` now name dotfiles-mcp ShaderModule instead of retired `shader-mcp`/CRTty surfaces, with a repo-smoke grep to keep them clean.
- [x] [P2][S] dotfiles-mcp README snapshot-count guard — refreshed the module README to `434` tools / `41` modules / `25` resources / `13` prompts and taught `validate-tool-count-claims.sh` to exact-check that block against `snapshots/contract/overview.json`.
- [x] [P2][S] dotfiles-mcp shader review-doc guard — `mcp/dotfiles-mcp/REVIEW.md` now uses Kitty DarkWindow shader wording, and the active shader stale-term smoke test covers the review guide too.
- [x] [P2][S] dotfiles-mcp README retired-link guard — `mcp/dotfiles-mcp/README.md` now points to in-tree Systemd/Tmux/Process modules instead of retired standalone repo URLs, with a repo-smoke check to keep those URLs out.
