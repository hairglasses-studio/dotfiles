...
- [ ] [P3][S] Submit to PulseMCP, Glama, MCP Market directories — standalone mirror/release state repaired on 2026-04-23: `hairglasses-studio/dotfiles-mcp` is public/non-archived, tagged and released at `v1.1.0`, `go install ...@latest` resolves the release, Glama metadata is committed, projection status is `status=in_sync`, and a public MCPB registry artifact is released at `mcpb-2.2.0` with `mcp-publisher validate` passing. Remaining listing work is external: authenticated Official MCP Registry publish, PulseMCP ingestion after that publish, MCP Market browser review form, and optional Glama browser submission or crawler pickup.
