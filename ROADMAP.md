...
- [ ] [P2][M] `mcp/dotfiles-mcp` Go tests fail on Python 3.14 — `TestDesktopSessionListWindows_UnsupportedWithoutDBus` asserts the error substring `DBUS_SESSION_BUS_ADDRESS` but pyatspi 2.x raises an `AttributeError: type object 'object' has no attribute '__getattr__'` under Python 3.14 (`pyatspi/registry.py:123`). Either pin the test's error-substring tolerant or wait for pyatspi 3.x.
- [ ] [P2][S] `TestDotfilesWorkstationDiagnosticsDegradedWhenDesktopReadinessFails` is brittle — it asserts the presence of exactly one `desktop.hyprland` issue but the diagnostic now surfaces multiple degraded components (accessibility, ironbar, notifications, shader, shell) when several desktop-readiness probes fail. Loosen the assertion to "at least one hyprland issue in the list" and verify a known-shape issue, not an exact-match list.
