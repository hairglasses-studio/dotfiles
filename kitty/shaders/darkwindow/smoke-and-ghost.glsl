...
float ghostBackground(vec2 uv, vec2 center) {
    ...
    // Add wavy bottom
    float wave = sin(d.x * 6.28 + x_Time) * 0.2;
...
float calculateSmoke(vec2 uv, vec2 sourcePos) {
    ...
    smokeUV.y -= x_Time * SMOKE_SPEED * (1.0 + verticalDisp);
    smokeUV.x += sin(x_Time * 0.5 + uv.y * 4.0) * 0.1;
    
    float n = noise(smokeUV) * 0.5 + 0.5;
    n += noise(smokeUV * 2.0 + x_Time * 0.1) * 0.25;
...
void windowShader(inout vec4 _wShaderOut) {
    ...
    float faceAccum = 0.0;
    float bgAccum = 0.0;
    float timeBlock = floor(x_Time / FACE_DURATION);
    ...
    if (smokePresence > 0.2) {
        for (int i = 0; i < FACE_COUNT; i++) {
            ...
            float faceTime = mod(x_Time, FACE_DURATION);
            ...
            // Add face features
            float face = ghostFace(uv, facePos, x_Time, timeBlock + float(i) * 100.0) * fadeFactor;
