...
// (Post-FX) — Computer glitch variant 3
...
void windowShader(inout vec4 _wShaderOut) {
    ...
    // Channel shifting
    vec3 rChannel = x_Texture(glitchUV + vec2(0.01, 0.0)).r * vec3(1.0, 0.0, 0.0);
    vec3 gChannel = x_Texture(glitchUV).g * vec3(0.0, 1.0, 0.0);
    vec3 bChannel = x_Texture(glitchUV - vec2(0.01, 0.0)).b * vec3(0.0, 0.0, 1.0);
