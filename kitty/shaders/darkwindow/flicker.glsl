...
void windowShader(inout vec4 _wShaderOut) {
    ...
    float timeScale = floor(x_Time * 2.0);
    ...
    float r = x_Texture(vec2(uv.x + splitStrength, uv.y)).r;
    float g = x_Texture(uv).g;
    float b = x_Texture(vec2(uv.x - splitStrength, uv.y)).b;
    ...
    float vSync = sin(uv.y * 50.0 + x_Time * 5.0) * randomTrigger * 0.01;
    uv.x += vSync;
