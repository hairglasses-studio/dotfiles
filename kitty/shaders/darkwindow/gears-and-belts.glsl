...
#define antialiasing(n) n/min(x_WindowSize.y,x_WindowSize.x)
...
float innerGear(vec2 p, float dir){
    p*=Rot(radians(-x_Time*45.+45.)*dir);
    vec2 prevP = p;

    //p*=Rot(radians(x_Time*45.+20.));
    p = DF(p,7.);
    p-=vec2(0.24);
    p*=Rot(deg45);
...
vec3 gear(vec2 p, vec3 col, float dir){
    ...
    p*=Rot(radians(x_Time*45.+13.)*-dir);
    p = DF(p,7.);
    p-=vec2(0.23);
    p*=Rot(deg45);
    ...
    p*=Rot(radians(x_Time*30.-30.)*dir);
    p = DF(p,6.);
    p-=vec2(0.14);
    p*=Rot(radians(45.));
    ...
    p*=Rot(radians(x_Time*25.+30.)*-dir);
...
vec3 item3(vec2 p, vec3 col, float dir){
    ...
    p*=Rot(radians(sin(x_Time*dir)*120.));
    vec2 prevP = p;
