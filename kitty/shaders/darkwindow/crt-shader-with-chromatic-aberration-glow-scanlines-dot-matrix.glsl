...
void windowShader(inout vec4 _wShaderOut) {
    ...
    vec4 colR = x_Texture(vec2(uv.x-ABBERATION_FACTOR*amount / x_WindowSize.x, uv.y));
    vec4 colG = x_Texture(uv);
    vec4 colB = x_Texture(vec2(uv.x+ABBERATION_FACTOR*amount / x_WindowSize.x, uv.y));
    ...
    if (source.x > DIM_CUTOFF) {
        ...
        vec2 step = vec2(1.414) / x_WindowSize;
        vec3 glow = vec3(0.0);
        for (int i = 0; i < 24; i++) {
            vec3 s = samples[i];
            float weight = s.z;
            vec4 c = toOklab(x_Texture(uv + s.xy * step));
            if (c.x > DIM_CUTOFF) {
    ...
    // Apply Scanlines
    float scanline = abs(sin(x_PixelPos.y) * SCANLINE_DENSITY * SCANLINE_INTENSITY);

    // Apply Dot Matrix
    vec2 mask_pos = x_PixelPos.xy * MASK_SIZE;
