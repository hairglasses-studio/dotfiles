void windowShader(inout vec4 color) {
    ...
    // Fetch the video frame (assume x_Texture is the video texture)
    vec4 videoColor = x_Texture(uv);

    // --- Animated Film Grain ---
    float grain = fract(sin(dot(x_PixelPos.xy + x_Time * 50.0, vec2(12.9898, 78.233))) * 43758.5453);
    grain *= mix(0.9, 1.0, sin(x_Time * 0.5 + uv.x * 50.0)); // Slow, animated grain
    vec3 grainEffect = videoColor.rgb * (1.0 - grain * 0.12); // Enhanced grain intensity

    // --- Reduced Frequency Scratches ---
    float scratchChance = step(0.995, fract(sin(uv.x * 200.0 + x_Time * 20.0) * 43758.5453123)); // 25% frequency
    float scratchIntensity = smoothstep(0.0, 0.02, abs(sin(uv.y * 50.0 + x_Time * 3.0)));
    float scratch = scratchChance * scratchIntensity * step(0.9, fract(sin(x_Time * 20.0 + uv.x * 500.0) * 43758.545));
    vec3 scratchEffect = vec3(1.0) - vec3(scratch * 0.7); // Overlay random vertical scratches

    // --- Localized Squiggly Film Blotches ---
    float blotch = 0.0;
    
    float blotch_time = floor(x_Time * 20.0);
