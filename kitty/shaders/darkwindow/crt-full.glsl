...
vec3 CrtsFetch(vec2 uv) {
  return FromSrgb(x_Texture(uv.xy).rgb);
}
...
vec3 sampleBloom(vec2 uv, vec2 texelSize) {
  ...
  bloom += FromSrgb(x_Texture(uv + vec2(-spread, -spread) * texelSize).rgb);
  bloom += FromSrgb(x_Texture(uv + vec2( spread, -spread) * texelSize).rgb);
  bloom += FromSrgb(x_Texture(uv + vec2(-spread,  spread) * texelSize).rgb);
  bloom += FromSrgb(x_Texture(uv + vec2( spread,  spread) * texelSize).rgb);
  bloom += FromSrgb(x_Texture(uv + vec2( 0.0,    -spread) * texelSize).rgb);
  bloom += FromSrgb(x_Texture(uv + vec2( 0.0,     spread) * texelSize).rgb);
  bloom += FromSrgb(x_Texture(uv + vec2(-spread,   0.0)   * texelSize).rgb);
  bloom += FromSrgb(x_Texture(uv + vec2( spread,   0.0)   * texelSize).rgb);
  return bloom / 8.0;
}
...
void windowShader(inout vec4 _wShaderOut) {
  ...
  vec2 jitteredCoord = x_PixelPos.xy;
  #ifdef ENABLE_JITTER
  {
      float scanline = floor(x_PixelPos.y);
      float jit = (hash(vec2(scanline, x_Time * 60.0)) - 0.5) * 2.0;
      jitteredCoord.x += jit * JITTER_AMOUNT * x_WindowSize.x;
  }
  #endif
  ...
  _wShaderOut.rgb += GLOW_COLOR * luma * GREEN_GLOW;
