...
void windowShader(inout vec4 _wShaderOut) {
    ...
    vec3 text;
    text.r = x_Texture(curvedUV + vec2(aberration, 0.0)).r;
    text.g = x_Texture(curvedUV).g;
    text.b = x_Texture(curvedUV - vec2(aberration, 0.0)).b;
    
    // Retro Ghosting (Slight horizontal bleed)
    vec3 ghost = x_Texture(curvedUV - vec2(0.002, 0.0)).rgb * 0.3;
    text += ghost * UI_TINT;
    ...
    float mask = sin(x_PixelPos.x * 1.5) * 0.1 + 0.9;
    finalColor *= mask;
    ...
    bloom += x_Texture(curvedUV + vec2(b_offset, 0.0)).rgb;
    bloom += x_Texture(curvedUV - vec2(b_offset, 0.0)).rgb;
