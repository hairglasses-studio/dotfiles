...
void windowShader(inout vec4 _wShaderOut)
{
    ...
    float time = x_Time * palpitationFrequency;
    float palpitateScale = 0.10 + palpitationAmplitude * sin(time);

    // Draw x_Texture multiple times bigger but behind
    float scale = 1.0 * palpitateScale; // Adjust this value to control the overall scale of x_Texture
    float r_background = x_Texture(uv + vec2(aberrationOffset * glitchX, 0.001) * scale).r;
    float g_background = x_Texture(uv).g;
    float b_background = x_Texture(uv - vec2(aberrationOffset * glitchY, 0.0) * scale).b;

    // Blend the background color with the original color from x_Texture
    ...
    time = x_Time * 0.5; // Adjust the multiplier for scan line movement speed
    float scanLine = mod(floor((uv.y + x_Time * 0.2) * x_WindowSize.y / scanLineSpacing), 2.0);
    finalColor *= 1.0 - scanLineIntensity * scanLine;

    // Add film reel noise
    time = x_Time * 0.3; // Adjust the multiplier for speed
