...
void windowShader(inout vec4 _wShaderOut) {
    ...
    vec2 normCoord = x_PixelPos * scale - normOffset;
    ...
    float v1v = sin(normCoord.x * 10.0 + x_Time);
    float v2v = sin(normCoord.y * 10.0 + x_Time * 4.5);
    float v3v = sin((normCoord.x + normCoord.y) * 10.0 + x_Time * 0.5);
    float v4v = sin(length(normCoord) * 10.0 + x_Time * 2.0);
