...
void windowShader(inout vec4 _wShaderOut)
{
    ...
    float timeSinceMove = x_Time - x_Time;
    ...
    for (int i = 0; i < NUM_BIRDS; i++) {
        ...
        if (movementLength < 0.001 || flightProgress >= 1.0) {
            ...
            float orbitTime = x_Time * 2.0 + birdOffset;
            ...
            float targetOrbitTime = x_Time * 2.0 + birdOffset;
        ...
        for (float trailStep = 1.0; trailStep <= numTrailSegments; trailStep++) {
            float trailTime = x_Time - (trailStep * 0.09); // Look back in time (3x further)
            float trailTimeSinceMove = trailTime - x_Time;
            ...
            if (trailTimeSinceMove >= 0.0) { // Only draw if this time point is after cursor moved
                ...
                float colorTime = x_Time * 0.5 + birdId * 0.2 + trailStep * 0.1;
