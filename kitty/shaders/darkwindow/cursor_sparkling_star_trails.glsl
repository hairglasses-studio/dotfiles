...
void windowShader(inout vec4 _wShaderOut)
{
    ...
    if (movementLength > 0.01) {
        ...
        for (int i = 0; i < MAX_TRAILS; i++) {
            ...
            float randOffset = hash(particleId + x_Time * 1000.0);
            float randX = hash(particleId * 2.0 + x_Time * 1000.0) - 0.5;
            float randY = hash(particleId * 3.0 + x_Time * 1000.0) - 0.5;
            ...
            if (particleProgress > 0.0) {
                ...
                float colorChance = hash(particleId * 5.0 + x_Time * 1000.0);
                vec3 trailColor;
