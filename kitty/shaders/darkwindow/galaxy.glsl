...
float field(in vec3 position) {	
  float strength = 7.0 + 0.03 * log(1.0e-6 + fract(sin(x_Time) * 373.11));
  ...
  for (int i = 0; i < 6; ++i) {
    ...
    position = abs(position) / magnitude + vec3(-0.5, -0.8 + 0.1 * sin(-x_Time * 0.1 + 2.0), -1.1 + 0.3 * cos(x_Time * 0.3));
...
void windowShader(inout vec4 color) {
  ...
  cameraPosition.x += transverseMotion * cos(0.01 * x_Time) + 0.001 * x_Time;
  cameraPosition.y += transverseMotion * sin(0.01 * x_Time) + 0.001 * x_Time;
  cameraPosition.z += 0.003 * x_Time;	
  ...
  backgroundColor.bg = mix(backgroundColor.gb, backgroundColor.bg, 0.5 * (cos(x_Time * 0.01) + 1.0));	

  vec2 terminalUV = x_PixelPos.xy / x_WindowSize;
  vec4 terminalColor = x_Texture(terminalUV);
