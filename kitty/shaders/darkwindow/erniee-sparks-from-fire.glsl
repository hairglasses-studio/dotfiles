...
float layeredNoise1_2(in vec2 uv, in float sizeMod, in float alphaMod, in int layers, in float animation)
{
    ...
    for (int i = 0; i < layers; i++)
    {
        ...
        //Adding noise with movement
     	  noise += noise1_2(uv * size + x_Time * animation * 8.0 * MOVEMENT_DIRECTION * MOVEMENT_SPEED + offset) * alpha;
        alpha *= alphaMod;
        size *= sizeMod;
    }
...
{
 	return x_Time * ANIMATION_SPEED * (hash1_2(uv) - 0.5) * 2.0;   
}
...
vec3 fireParticles(in vec2 uv, in vec2 originalUV)
{
    ...
    tempUV += -(noise2_2(uv * 3.0 + x_Time) - 0.5) * 0.07;
...
{ 
    ...
    for (int i = 0; i < layers; i++)
    {
        ...
        bokehUV = (uv * size + x_Time * MOVEMENT_DIRECTION * MOVEMENT_SPEED) + offset + noiseOffset; 
...
void windowShader(inout vec4 color) {
    ...
    float smokeIntensity = layeredNoise1_2(uv * 10.0 + x_Time * 4.0 * MOVEMENT_DIRECTION * MOVEMENT_SPEED, 1.7, 0.7, 6, 0.2);
    ...
    smoke *= pow(layeredNoise1_2(uv * 4.0 + x_Time * 0.5 * MOVEMENT_DIRECTION * MOVEMENT_SPEED, 1.8, 0.5, 3, 0.2), 2.0) * 1.5;
