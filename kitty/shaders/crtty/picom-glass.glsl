...
void main() {
    ...
    float animation_progress = 0.5 + 0.5 * cos(u_time * 1.047);
    ...
    vec4 out_color;
    out_color.r = texture(u_input, sampleUv + ca_offset).r;
    out_color.g = texture(u_input, sampleUv).g;
    out_color.b = texture(u_input, sampleUv - ca_offset).b;
    out_color.a = texture(u_input, sampleUv).a;
