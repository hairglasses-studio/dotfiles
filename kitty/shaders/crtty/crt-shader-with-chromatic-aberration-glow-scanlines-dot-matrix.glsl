...
float offsetFunction(float u_time) {
    float amount = 1.0;
    const float periods[4] = {6.0, 16.0, 19.0, 27.0};
    for (int i = 0; i < 4; i++) {
        amount *= 1.0 + 0.5 * sin(u_time*periods[i]);
    }
...
void main() {
    ...
    float amount = offsetFunction(u_time);
    vec4 colR = texture(u_input, vec2(uv.x-ABBERATION_FACTOR*amount / u_resolution.x, uv.y));
    vec4 colG = texture(u_input, uv);
    vec4 colB = texture(u_input, vec2(uv.x+ABBERATION_FACTOR*amount / u_resolution.x, uv.y));
    ...
    if (source.x > DIM_CUTOFF) {
        ...
        vec2 step = vec2(1.414) / u_resolution;
        vec3 glow = vec3(0.0);
        for (int i = 0; i < 24; i++) {
            vec3 s = samples[i];
            float weight = s.z;
            vec4 c = toOklab(texture(u_input, uv + s.xy * step));
            if (c.x > DIM_CUTOFF) {
    ...
    // Apply Scanlines
    float scanline = abs(sin(gl_FragCoord.xy.y) * SCANLINE_DENSITY * SCANLINE_INTENSITY);

    // Apply Dot Matrix
    vec2 mask_pos = gl_FragCoord.xy.xy * MASK_SIZE;
    ...
    o_color = vec4(with_scanline, final_alpha);
}
