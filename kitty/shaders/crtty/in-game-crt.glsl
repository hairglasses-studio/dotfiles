...
void main() {
    ...
    o_color.r = texture(u_input, vec2(uv.x + 0.0003 * COLOR_FRINGING_SPREAD, uv.y + 0.0003 * COLOR_FRINGING_SPREAD)).x;
    o_color.g = texture(u_input, vec2(uv.x + 0.0000 * COLOR_FRINGING_SPREAD, uv.y - 0.0006 * COLOR_FRINGING_SPREAD)).y;
    o_color.b = texture(u_input, vec2(uv.x - 0.0006 * COLOR_FRINGING_SPREAD, uv.y + 0.0000 * COLOR_FRINGING_SPREAD)).z;
    o_color.a = texture(u_input, uv).a;


    // Add faint ghost images
    o_color.r += 0.04 * GHOSTING_STRENGTH * texture(u_input, GHOSTING_SPREAD * vec2(+0.025, -0.027) + uv.xy).x;
    o_color.g += 0.02 * GHOSTING_STRENGTH * texture(u_input, GHOSTING_SPREAD * vec2(-0.022, -0.020) + uv.xy).y;
    o_color.b += 0.04 * GHOSTING_STRENGTH * texture(u_input, GHOSTING_SPREAD * vec2(-0.020, -0.018) + uv.xy).z;


    // Quadratically darken everything
    o_color.rgb = mix(o_color.rgb, o_color.rgb*o_color.rgb, DARKEN_MIX);


    // Vignette effect
    // NOTE: Clamp necessary because of curve effect
    o_color.rgb *= VIGNETTE_BRIGHTNESS * pow(clamp(uv.x * uv.y * (1.0-uv.x) * (1.0-uv.y), 0.0, 1.0), VIGNETTE_SPREAD);


    // Tint all colors
    o_color.rgb *= vec3(TINT);
    ...
    o_color.rgb *= mix(
        1.0,
        SCAN_LINES_VARIANCE/2.0*(1.0 + sin(2*PI* uv.y * u_resolution.y/SCAN_LINES_PERIOD)),
        SCAN_LINES_STRENGTH
    );


    // Add aperture grille
    int apertureGrilleStep = int(8 * mod(gl_FragCoord.xy.x, APERTURE_GRILLE_PERIOD) / APERTURE_GRILLE_PERIOD);
    float apertureGrilleMask;

    if (apertureGrilleStep < 3)
        apertureGrilleMask = 0.0;
    else if (apertureGrilleStep < 4)
        apertureGrilleMask = mod(8*gl_FragCoord.xy.x, APERTURE_GRILLE_PERIOD) / APERTURE_GRILLE_PERIOD;
    else if (apertureGrilleStep < 7)
        apertureGrilleMask = 1.0;
    else if (apertureGrilleStep < 8)
        apertureGrilleMask = mod(-8*gl_FragCoord.xy.x, APERTURE_GRILLE_PERIOD) / APERTURE_GRILLE_PERIOD;

    o_color.rgb *= 1.0 - APERTURE_GRILLE_STRENGTH*apertureGrilleMask;


    // Add flicker
    o_color *= 1.0 - FLICKER_STRENGTH/2.0*(1.0 + sin(2*PI*FLICKER_FREQUENCY*u_time));


    // Add noise
    // NOTE: Hard-coded noise distributions
    float noise = smoothstep(0.4, 0.6, gold_v2_noise(gl_FragCoord.xy.xy, fract(u_time*0.001)));
    o_color.rgb *= clamp(noise + 1.0 - NOISE_CONTENT_STRENGTH, 0.0, 1.0);
    o_color.rgb = clamp(o_color.rgb + noise * NOISE_UNIFORM_STRENGTH / 100, 0.0, 1.0);
...
#ifdef BLOOM_SPREAD
    // Add bloom
    vec2 step = BLOOM_SPREAD * vec2(1.414) / u_resolution;

    for (int i = 0; i < 24; i++) {
        vec3 bloomSample = bloomSamples[i];
        vec4 neighbor = texture(u_input, uv + bloomSample.xy * step);
        float luminance = 0.299 * neighbor.r + 0.587 * neighbor.g + 0.114 * neighbor.b;

        o_color += luminance * bloomSample.z * neighbor * BLOOM_STRENGTH;
    }

    o_color = clamp(o_color, 0.0, 1.0);
#endif


    // Set background opacity
    o_color = vec4(o_color.rgb*o_color.a, BACKGROUND_OPACITY);
}
