...
void main()
{
    ...
    vec4 newColor = vec4(o_color);

    float gradientFactor = (vu.y + 1.0) * 0.5; // Gradient across vertical position
    // float timeComponent = sin(u_time) * 0.95 + 0.05; // Oscillates between 0 and 1
    float timeComponent = sin(u_time); // Oscillates between 0 and 1
    ...
    // newColor = mix(o_color, newColor, OPACITY);
    o_color = mix(o_color, newColor, step(sdfCurrentCursor, easedProgress * lineLength));
}
