...
void main() {
    ...
    // Channel shifting
    vec3 rChannel = texture(u_input, glitchUV + vec2(0.01 * glitchBlock, 0.0)).r * vec3(1.0, 0.0, 0.0);
    vec3 gChannel = texture(u_input, glitchUV).g * vec3(0.0, 1.0, 0.0);
    vec3 bChannel = texture(u_input, glitchUV - vec2(0.01 * glitchBlock, 0.0)).b * vec3(0.0, 0.0, 1.0);
