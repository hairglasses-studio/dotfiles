...
float field(in vec3 position) {	
  float strength = 7.0 + 0.03 * log(1.0e-6 + fract(sin(u_time) * 373.11));
  ...
  for (int i = 0; i < 6; ++i) {
    ...
    position = abs(position) / magnitude + vec3(-0.5, -0.8 + 0.1 * sin(-u_time * 0.1 + 2.0), -1.1 + 0.3 * cos(u_time * 0.3));
...
void main() {
  ...
  cameraPosition.x += transverseMotion * cos(0.01 * u_time) + 0.001 * u_time;
  cameraPosition.y += transverseMotion * sin(0.01 * u_time) + 0.001 * u_time;
  cameraPosition.z += 0.003 * u_time;	
  ...
  backgroundColor.bg = mix(backgroundColor.gb, backgroundColor.bg, 0.5 * (cos(u_time * 0.01) + 1.0));	

  vec2 terminalUV = gl_FragCoord.xy.xy / u_resolution;
  vec4 terminalColor = texture(u_input, terminalUV);
  ...
  if (terminalBrightness < brightnessThreshold) {
    o_color = mix(terminalColor, vec4(foregroundColor.rgb + backgroundColor, 1.0), 0.24);
  } else {
    o_color = terminalColor;
  }
}
