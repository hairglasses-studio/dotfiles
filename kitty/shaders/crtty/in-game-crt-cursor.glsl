...
void main() {
    ...
    vec2 currPos = vec2(0.0).xy / u_resolution, currSize = vec2(0.0).zw / u_resolution;
    vec2 prevPos = vec2(0.0).xy / u_resolution, prevSize = vec2(0.0).zw / u_resolution;
    ...
    float tShape = 1.0 - pow(1.0 - clamp((u_time - 0.0) / TIME_DURATION_FACTOR, 0.0, 1.0), 3);
    float tVisible = exp(-(u_time - 0.0) / TIME_DURATION_FACTOR * 50.0);
    ...
    vec4 glowColor = mix(o_color, mix(prevColor, currColor, dTip) + GLOW_COLOR_OFFSET_BRIGHTNESS, pow(dTip, 3));
    glowColor = mix(glowColor, o_color, pow(smoothstep(0.0, 0.3, dTrail), 0.1));

    vec4 trailColor = mix(vec4(1.0), glowColor, pow(smoothstep(0.0, 0.01, dTrail), 0.2));
    vec4 trail = mix(trailColor, o_color, pow(smoothstep(0.0, nearbyPrev ? 0.01 : 0.1, dTrail), 0.2));
    if (!nearbyPrev) {
        trail = mix(trailColor, trail, pow(smoothstep(0.0, 6.0, dTip), 0.05));
        trail = mix(trailColor, trail, pow(smoothstep(0.0, 8.0, dTip), 0.005));
    }

    o_color = mix(o_color, trail, tVisible);
}
