...
void main() {
    ...
    vec3 col;
    col.r = texture(u_input, vec2(uv.x-ABBERATION_FACTOR*amount / u_resolution.x, uv.y) ).r;
    col.g = texture(u_input, uv ).g;
    col.b = texture(u_input, vec2(uv.x+ABBERATION_FACTOR*amount / u_resolution.x, uv.y) ).b;
