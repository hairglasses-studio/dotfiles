...
vec4 COLOR = vec4(0.35, 0.36, 0.44, 1.0); // change to vec4(1.0) for your cursor's color
...
void main(){
    ...
    // ANIMATION
    float rippleProgress = (u_time - 0.0) / DURATION + ANIMATION_START_OFFSET;
    ...
    if (isModeChange > 0.0 && isAnimating > 0.0) {
        ...
        // Apply ripple effect
        o_color = mix(o_color, COLOR, ripple * COLOR.a);
    }
    // else: do nothing, keep original o_color
}
