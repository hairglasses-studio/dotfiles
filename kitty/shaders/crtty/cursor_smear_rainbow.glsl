...
void main() {
    ...
    float baseProgress = clamp((u_time - 0.0) / DURATION, 0.0, 1.0);

    vec2 uv = gl_FragCoord.xy / u_resolution;
    vec4 background = texture(u_input, uv);
    ...
    float aaWidth = scale;
    vec2 normOffset = u_resolution * invResY;

    // Normalize cursor positions and sizes to screen-independent coordinates
    vec2 currentPos = vec2(0.0).xy * scale - normOffset;
    vec2 previousPos = vec2(0.0).xy * scale - normOffset;
    vec2 currentSize = vec2(0.0).zw * scale;
    vec2 previousSize = vec2(0.0).zw * scale;
    ...
    vec2 normCoord = gl_FragCoord.xy * scale - normOffset;
    ...
    float v1v = sin(normCoord.x * 10.0 + u_time);
    float v2v = sin(normCoord.y * 10.0 + u_time * 4.5);
    float v3v = sin((normCoord.x + normCoord.y) * 10.0 + u_time * 0.5);
    float v4v = sin(length(normCoord) * 10.0 + u_time * 2.0);
    ...
    vec4 originalColor = o_color;
    o_color.rgb = mix(o_color.rgb, enhancedColor.rgb, alpha);

    // Remove trail where it overlaps with current cursor
    o_color.rgb = mix(o_color.rgb, originalColor.rgb, step(sdfCurrentCursor, 0.0));
}
