...
void main() {
    ...
    if (curvedUV.x < 0.0 || curvedUV.x > 1.0 || curvedUV.y < 0.0 || curvedUV.y > 1.0) {
        o_color = vec4(0.0, 0.0, 0.0, 1.0); return;
    }
    ...
    vec3 text;
    text.r = texture(u_input, curvedUV + vec2(aberration, 0.0)).r;
    text.g = texture(u_input, curvedUV).g;
    text.b = texture(u_input, curvedUV - vec2(aberration, 0.0)).b;
    
    // Retro Ghosting (Slight horizontal bleed)
    vec3 ghost = texture(u_input, curvedUV - vec2(0.002, 0.0)).rgb * 0.3;
    text += ghost * UI_TINT;
    ...
    float mask = sin(gl_FragCoord.xy.x * 1.5) * 0.1 + 0.9;
    finalColor *= mask;
    ...
    bloom += texture(u_input, curvedUV + vec2(b_offset, 0.0)).rgb;
    bloom += texture(u_input, curvedUV - vec2(b_offset, 0.0)).rgb;
