...
void main() {
    ...
    // Gentle unevenness
    float wobble = fbm(vec2(gl_FragCoord.xy.x * 0.005, gl_FragCoord.xy.y * 0.004));
    grad += (wobble - 0.5) * 0.1;

    // Fine granulation from pigment settling
    float granulate = vnoise(gl_FragCoord.xy * 0.02);
    ...
    if (isBg > 0.5) {
        if (inPaint > 0.5) {
            ...
            result = mix(vec4(0.0, 0.0, 0.0, 1.0), washColor, 0.6 * washStrength);
