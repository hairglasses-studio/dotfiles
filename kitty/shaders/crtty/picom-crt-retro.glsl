        ...
        for (int j = 0; j < downscale_factor; j++) {
            for (int i = 0; i < downscale_factor; i++) {
                ...
                if (sUv.x >= 0.0 && sUv.x <= 1.0 && sUv.y >= 0.0 && sUv.y <= 1.0)
                    c += texture(u_input, sUv);
            }
        }
        c /= float(downscale_factor * downscale_factor);
    }

    vec4 c_right = texture(u_input, curvedUv + vec2(distortion_offset * px.x, 0.0));
    vec4 c_left  = texture(u_input, curvedUv - vec2(distortion_offset * px.x, 0.0));
    c = vec4(c_left.r, c.g, c_right.b, c.a);

    float scanY = sin(2.0 * PI * sc_freq * gl_FragCoord.xy.y) / (2.0 / sc_intensity) + 1.0 - sc_intensity / 2.0;
    c.rgb *= scanY;

    if (grid) {
        float scanX = sin(2.0 * PI * sc_freq * gl_FragCoord.xy.x) / (2.0 / sc_intensity) + 1.0 - sc_intensity / 2.0;
        c.rgb *= scanX;
    }
