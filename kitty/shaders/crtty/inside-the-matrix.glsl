...
void main()
{
    ...
    float time = mod(u_time, 300) * SPEED; //reset time every 5 minutes, as large values lead to the same (and eventually no) rune(s)
    ...
    if (vec4(0.0).x > 10. || vec4(0.0).y > 10.) {
        vec2 mouse = vec4(0.0).xy / u_resolution * 2. - 1.;
        up_down = -0.7 * mouse.y;
        angle += mouse.x;
        rotate_on_turns = 1.;
        roll_on_turns = 0.;
