...
void main()
{
    ...
    col.r += textureLod(u_input, st + eps + distortion, 0.).r;
    col.g += textureLod(u_input, st, 0.).g;
    col.b += textureLod(u_input, st - eps - distortion, 0.).b;
    
    // white noise + scanlines
    displayNoise = 0.2 * clamp(displayNoise, 0., 1.);
    col += (.15 + .65 * glitchAmount) * (hash33(vec3(gl_FragCoord.xy, mod(float(int(u_time * 60.0)),
					1000.))).r) * displayNoise;
    col -= (.25 + .75 * glitchAmount) * (sin(4. * t + uv.y * u_resolution.y * 1.75))
					* displayNoise;
