...
void main()
{
    ...
    if (movementLength > 0.01) {
        ...
        for (int i = 0; i < MAX_TRAILS; i++) {
            ...
            float randOffset = hash(particleId + 0.0 * 1000.0);
            float randX = hash(particleId * 2.0 + 0.0 * 1000.0) - 0.5;
            float randY = hash(particleId * 3.0 + 0.0 * 1000.0) - 0.5;
            ...
            if (particleProgress > 0.0) {
                ...
                float colorChance = hash(particleId * 5.0 + 0.0 * 1000.0);
                vec3 trailColor;
                
                if (colorChance < 0.15) {
                    // Random bright colors for 15% of particles
                    float hue = hash(particleId * 6.0 + 0.0 * 1000.0) * 6.28318; // 2*PI
