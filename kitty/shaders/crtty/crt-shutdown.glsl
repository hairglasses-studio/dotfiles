...
// Triggered by swapping custom-shader to this file; u_time resets to 0.
//
...
void main() {
    ...
    if (t > T_HCOLLAPSE) {
        ...
        o_color = vec4(PHOSPHOR_COLOR * dot, 1.0);
        return;
    }
    ...
    if (t > T_VCOLLAPSE) {
        ...
        vec3 col = texture(u_input, sampleUV).rgb;

        // Concentrate brightness as beam compresses
        float bright = 1.5 + p * 2.0;
        col *= bright;

        o_color = vec4(col * hmask * vmask, 1.0);
        return;
    }
