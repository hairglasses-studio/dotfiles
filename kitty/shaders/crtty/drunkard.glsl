...
void main()
{
    ...
    float time = ANIMATE ? u_time * SPEED : 0.0;
    ...
    if (ABERRATION) {
        col.r = texture(u_input, uv + vec2(noise * (1.0 + ABERRATION_DELTA))).r;
        col.g = texture(u_input, uv + vec2(noise)).g;
        col.b = texture(u_input, uv + vec2(noise * (1.0 - ABERRATION_DELTA))).b;
    } else {
        vec2 distortedUV = uv + vec2(noise);
        col = texture(u_input, distortedUV).rgb;
    }
