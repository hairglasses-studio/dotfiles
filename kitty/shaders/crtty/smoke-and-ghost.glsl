...
float ghostBackground(vec2 uv, vec2 center) {
    ...
    // Add wavy bottom
    float wave = sin(d.x * 6.28 + u_time) * 0.2;
...
float calculateSmoke(vec2 uv, vec2 sourcePos) {
    ...
    smokeUV.y -= u_time * SMOKE_SPEED * (1.0 + verticalDisp);
    smokeUV.x += sin(u_time * 0.5 + uv.y * 4.0) * 0.1;
    
    float n = noise(smokeUV) * 0.5 + 0.5;
    n += noise(smokeUV * 2.0 + u_time * 0.1) * 0.25;
...
void main() {
    ...
    vec4 originalColor = texture(u_input, uv);
    ...
    float faceAccum = 0.0;
    float bgAccum = 0.0;
    float timeBlock = floor(u_time / FACE_DURATION);
    ...
    if (smokePresence > 0.2) {
        for (int i = 0; i < FACE_COUNT; i++) {
            ...
            float faceTime = mod(u_time, FACE_DURATION);
            ...
            // Add face features
            float face = ghostFace(uv, facePos, u_time, timeBlock + float(i) * 100.0) * fadeFactor;
