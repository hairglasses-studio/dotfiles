...
void mainImage(out vec4 o_color, vec2 gl_FragCoord.xy) {
    vec3 v = vec3(gl_FragCoord.xy, 1) / vec3(u_resolution, 1.0) - .5;
    // vec3 s = .5 / abs(v);
    // scale?
    vec3 s = .9 / abs(v);
    s.z = min(s.y, s.x);
    ...
    vec3 j = fract(i);
    i -= j;
    vec3 p = vec3(9, int(u_time * SPEED_MULTIPLIER * (9. + 8. * sin(i).x)), 0) + i;
    vec3 col = o_color.rgb;
    col.g = R / s.z;
    p *= j;
