...
void main() {
    ...
    if (splitChance > 0.7) {
        ...
        pixelCoord.x += int(splitIntensity * 10.0 * sin(u_time * 50.0));
        
        // Direct pixel sampling with sharp shifts
        vec3 rChannel = vec3(texelFetch(u_input, pixelCoord + ivec2(2, 0), 0).r, 0.0, 0.0);
        vec3 gChannel = vec3(0.0, texelFetch(u_input, pixelCoord, 0).g, 0.0);
        vec3 bChannel = vec3(0.0, 0.0, texelFetch(u_input, pixelCoord - ivec2(2, 0), 0).b);
    ...
    vec3 rChannel = vec3(texelFetch(u_input, ivec2(gl_FragCoord.xy + vec2(1.0, 0.0)), 0).r, 0.0, 0.0);
    vec3 gChannel = vec3(0.0, texelFetch(u_input, ivec2(gl_FragCoord.xy), 0).g, 0.0);
    vec3 bChannel = vec3(0.0, 0.0, texelFetch(u_input, ivec2(gl_FragCoord.xy - vec2(1.0, 0.0)), 0).b);
